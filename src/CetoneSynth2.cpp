...
// Default polyphony voice count
constexpr auto DEFAULT_VOICE_COUNT = 10;
...
: DISTRHO::Plugin(pNumParameters, 0, 0)
{
...
#if 0	// This is not cross-platform. Disable.
	this->path[0] = '\0';
	...
	// Explicitly set block size and sample rate on initialization
	// NOTICE: Not all hosts calls blockSizeChanged()!
	this->setBlockSizeAndSampleRate(getBufferSize(), getSampleRate());

	//this->LoadPresets("autoload.fxb");	// Factory bank is no longer available.
	this->_Synth->SetProgram(0);
	//this->updateDisplay();

	// Enable polyphony explicitly
	this->_Synth->ChangeVoices(DEFAULT_VOICE_COUNT);
...
#if 1
	// DPF-specific BPM access. See DPF example "Metronome".
	// Requires DISTRHO_PLUGIN_WANT_TIMEPOS to be set to 1.
	const TimePosition& timePos(getTimePosition());
	if (timePos.playing && timePos.bbt.valid)
	{
		this->_Synth->SetTempo(static_cast<float>(timePos.bbt.beatsPerMinute));
	}
#else
...
#ifdef _WIN32	// TODO: Implement cross-platform CPU monitoring
...
float CS2::getParameter(VstInt32 index) const 
{
	if(index < 0 || index >= PARAM_COUNT)
		return 0.0f;
	else
...
#else
			memcpy(&(this->programs[this->curProgram]), data, sizeof(sPRG));
#endif
			this->_Synth->Recall();
			//if(this->editor->isOpen())
			//	((Gui*)this->editor)->ReadProgram();
...
#else
			memcpy(&(this->programs[0]), data, sizeof(sPRG) * 128);
#endif
			this->_Synth->Recall();
			//if(this->editor->isOpen())
			//	((Gui*)this->editor)->ReadProgram();
...
#else	// Set internal sampleRate. This is required by myProcess().
	this->sampleRate = sampleRate;
#endif
	this->_Synth->ReadProgram();
}

VstInt32 CS2::processEvents(const DISTRHO::MidiEvent* events, uint32_t eventCount)
{
#if 0
...
#else
	if (nr > -1)
		snprintf(name, kVstMaxProgNameLen, "CetoneSynth2 #%d", nr);
	else
		snprintf(name, kVstMaxProgNameLen, "CetoneSynth2");
#endif
...
char* CS2::GetFileName(const char *name)
{
	if(this->path[0] == '\0')
		return NULL;
...
void CS2::LoadPresets(const char *name)
{
